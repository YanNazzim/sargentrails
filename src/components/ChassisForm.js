...
const ChassisForm = ({ initialData }) => {
  ...
  const devices = [
    ...
    { code: "OLDNB8700", display: "NB8700 SVR (Old Style: Pre 04/14/25)", stile: "Wide" },
  ...
  const getPartsForCombination = (device, handing, selectedPrefixes, finish) => {
  ...
  // Using simple styling that inherits from the parent (App.css handles colors)
  ...
  return (
    <div className="content-transition">
      <h1 className="title">80/PE80 Series Chassis</h1>
      ...
      <form onSubmit={handleSubmit} className="part-form">
        ...
        <div className="form-group">
          ...
          <div className="checkbox-group" style={{ 
            ...
            {prefixes.map((prefix) => (
              <label key={prefix.code} style={{ display: 'flex', alignItems: 'center', cursor: 'pointer', color: 'var(--text-secondary)' }}>
                <input
                  type="checkbox"
                  value={prefix.code}
                ...
                />
                <span>
                  <strong style={{ color: 'var(--accent-cyan)' }}>({prefix.code})</strong> — {prefix.name}
                </span>
              </label>
            ))}
          </div>
        ...
        <div className="form-group">
          ...
          <Select
            ...
            value={finish.find((f) => f.value === formData.finish) || null}
      ...
      {partNumber && (
        <div className="result-container">
          <h2 className="title" style={{ fontSize: '1.2rem', marginBottom: '1rem' }}>Found Part Numbers:</h2>

          <div className="part-number">
            <p>
              <strong style={{ color: 'var(--accent-purple)' }}>Chassis:</strong>{" "}
              <span dangerouslySetInnerHTML={{ __html: relatedParts.chassis }} />
            </p>
            {relatedParts.innerChassis && (
              <p>
                <strong style={{ color: 'var(--accent-purple)' }}>Inner Chassis:</strong>{" "}
                <span dangerouslySetInnerHTML={{ __html: relatedParts.innerChassis }} />
              </p>
            )}
            <p>
              <strong style={{ color: 'var(--accent-purple)' }}>Chassis Cover:</strong>{" "}
              <span dangerouslySetInnerHTML={{ __html: relatedParts.chassisCover }} />
            </p>
          </div>
        </div>
      )}
